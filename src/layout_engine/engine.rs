...
impl LayoutEngine {
    ...
    fn move_focus_internal(
        ...
        let mut raise_windows = self.filter_active_workspace_windows(space, raise_windows);
    ...
    pub fn all_window_ids(&self) -> Vec<WindowId> {
        ...
        for (_, workspace) in self.virtual_workspace_manager.workspaces.iter() {
            windows.extend(workspace.windows());
        }
...
mod tests {
    ...
    fn fullscreen_test_engine(
        follows: bool,
    ) -> (LayoutEngine, SpaceId, WindowId, WindowId, Vec<SpaceId>, HashMap<SpaceId, CGPoint>) {
        let mut engine = test_engine();
        ...
        let window_b = WindowId::new(1, 2);
        let window_info = |wid| (wid, None, None, None, true, CGSize::new(0.0, 0.0), None, None);
        let _ = engine.handle_event(LayoutEvent::SpaceExposed(space, screen));
        let _ = engine.handle_event(LayoutEvent::WindowsOnScreenUpdated(
            space,
            1,
            vec![window_info(window_a), window_info(window_b)],
        ...
        let _ = engine.handle_event(LayoutEvent::WindowFocused(space, window_a));
        (engine, space, window_a, window_b, vec![space], HashMap::default())
    }
    ...
    fn move_focus_at_boundary_keeps_fullscreen_confined() {
        ...
        let _ = engine.handle_event(LayoutEvent::WindowFocused(space, window_b));
        let _ = engine.handle_command(Some(space), &spaces, &centers, LayoutCommand::ToggleFullscreen);
        assert_eq!(selection_flags(&engine, space), (true, false));
    ...
    fn move_focus_without_flag_leaves_fullscreen_in_place() {
        let (mut engine, space, window_a, window_b, spaces, centers) = fullscreen_test_engine(false);
        let _ = engine.handle_command(Some(space), &spaces, &centers, LayoutCommand::ToggleFullscreen);
    ...
    fn snapshot_roundtrip_preserves_layout() {
        ...
        // A floating window on the active workspace.
        engine.ensure_window_floating(space, w3);

        // Custom workspace names on two of the default workspaces.
        ...
        // A window living on a non-active workspace, to exercise window->workspace mapping.
        let _ = engine.handle_virtual_workspace_command(space, &LayoutCommand::SwitchToWorkspace(1));
        let _ = engine.handle_event(LayoutEvent::WindowsOnScreenUpdated(
            space,
            3,
            vec![window_info(w4)],
        ...
        let _ = engine.handle_virtual_workspace_command(space, &LayoutCommand::SwitchToWorkspace(0));
    ...
    fn engine_serialization_schema_stability() {
        // Guards against accidental serde renames on upstream rebases: these
        // top-level wire keys are what the persisted file is built around.
