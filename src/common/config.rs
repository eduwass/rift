...
fn default_scrolling_column_width_ratio() -> f64 { 0.9 }
...
pub struct LayoutSettings {
    ...
    /// When the focused window is fullscreen, directional focus moves exit fullscreen before
    /// focusing the neighbouring window instead of focusing a window hidden behind fullscreen. At
    /// the workspace boundary nothing happens — the fullscreen never hops displays. Default off
    /// (stock behavior).
    #[serde(default)]
